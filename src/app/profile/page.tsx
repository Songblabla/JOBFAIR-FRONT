...
const ProfilePage = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const userBookings = response.data.data.filter(booking => booking.user === user._id)
  ...
  const filteredBookings = useMemo(() => {
    ...
    return bookings
      .filter(booking => booking.company.name.toLowerCase().includes(bookingSearchTerm.toLowerCase()))
      .sort((a, b) => {
        if (sortBy === 'date') return new Date(a.bookingDate).getTime() - new Date(a.bookingDate).getTime()
          ...
          <Card className="lg:col-span-1 overflow-hidden" style={{ background: theme === 'dark' ? gradientStyle.dark : gradientStyle.light }}>
            <CardContent className="p-6">
              <div className="flex flex-col items-center space-y-4">
                ...
                <h2 className="text-xl sm:text-2xl font-semibold">{user?.name.split("@")[0]}</h2>
                ...
                <Card className="w-full bg-white dark:bg-gray-800">
                  <CardContent className="p-4">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      >
                        {revealJobFairCard ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                      </Button>
                    </div>
                    <div 
                      className={`transition-all duration-300 ${revealJobFairCard ? '' : 'blur-sm'}`}
                      onClick={() => !revealJobFairCard && setRevealJobFairCard(true)}
                    >
          ...
          <Card className="lg:col-span-2 bg-gray-50 dark:bg-gray-900">
            <CardContent className="p-4 sm:p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl sm:text-2xl font-semibold">Interview Bookings</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    {filteredBookings.map((booking) => (
                      <Button
                        key={booking._id}
                        variant={selectedBooking?._id === booking._id ? "default" : "outline"}
                      ...
                      {selectedBooking ? (
                        <div>
                          ...
                          <div className="flex items-center mb-2">
                            ...
                            <p className="text-sm">Time: {formatTime(selectedBooking.bookingDate)}</p>
                          </div>
                        </div>
                      ) : (
              ...
              >
                <CarouselContent>
                  {filteredCompanies.map((company) => (
                    <CarouselItem key={company._id} className="md:basis-1/2 lg:basis-1/3 xl:basis-1/4">
                      <div className="p-1">
                        <Card>
